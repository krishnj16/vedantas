    ...
    defineField({
      name: 'schedule', title: 'Weekday schedule', type: 'array', of: [defineArrayMember({
        type: 'object', fields: [
          ...
          defineField({ name: 'startTime', title: 'Start time', type: 'string', components: { input: TimeInput }, validation: (rule) => rule.required().regex(/^([01]\d|2[0-3]):[0-5]\d$/, { name: '24-hour time' }) }),
          defineField({ name: 'endTime', title: 'End time', type: 'string', components: { input: TimeInput }, validation: (rule) => rule.regex(/^([01]\d|2[0-3]):[0-5]\d$/, { name: '24-hour time' }).optional().custom((endTime, context) => {
            if (!endTime || typeof endTime !== 'string') return true;
            const startTime = context.parent?.startTime;
            return !startTime || endTime > startTime || 'End time must be later than the start time.';
          }) }),
